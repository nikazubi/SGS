...
public class ExportWordServiceImpl implements ExportWordService {
    ...
    private static final int[] FIRST_SEMESTER_MONTH_INT = {9, 11, 12};
    private static final String[] SECOND_SEMESTER_MONTH_NAMES = {"იანვარი-თებერვალი", "მარტი", "აპრილი", "მაისი", "ივნისი"};
    private static final int[] SECOND_SEMESTER_MONTH_INT = {1, 3, 4, 5, 6};
    ...
    public void exportSemesterGrades(Map<Student, Map<Subject, Map<Integer, BigDecimal>>> grades, String semester) {
        try (XWPFDocument document = new XWPFDocument()) {
            ...
            BigDecimal numOfPages = BigDecimal.valueOf(numberOfColumns).divide(BigDecimal.valueOf(NUMBER_OF_SUBJECTS_PER_PAGE), RoundingMode.CEILING);
            ...
            int[] monthsNum = "firstSemester".equals(semester)? FIRST_SEMESTER_MONTH_INT : SECOND_SEMESTER_MONTH_INT;

            for (int i = 0; i < numOfPages.longValue(); i++){
                 String[] headers = getColumnsForPage(arraySubjectNames, i);
                 String[][] studentData = new String[students.size()][headers.length];
                 for (int j = 0; j < students.size(); j ++) {
                     String studentName = students.get(j).getFirstName() + " " + students.get(j).getLastName();
                     String[] gradeArr= new String[headers.length + 1];
                     gradeArr[0] = studentName;
                     for (int k = 0; k < headers.length; k ++) {
                         Subject subject = subjectWithNames.get(headers[k]);
                         for (int m = 0; m < monthsNum.length; m ++) {
                             gradeArr[k + 1 + m] = grades.get(students.get(j)).get(subject).get(monthsNum[m]).toString();
                         }
                     }
                 }
                 createTable(document, headers, months, studentData);
                 if (i != numOfPages.longValue()) {
            ...
            // Save the document
            try (FileOutputStream out = new FileOutputStream("ComplexGridDocument.docx")) {
                document.write(out);
    ...
    private String[] getColumnsForPage(String[] cols, int page) {
        int startIdx = (page - 1) * NUMBER_OF_SUBJECTS_PER_PAGE;
