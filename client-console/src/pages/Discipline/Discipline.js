...
const Discipline = ({match}) => {
    ...
    const columns = useMemo(
        () => {
            ...
            const trimesterOngoingGrades = gradeData?.filter((grade) => grade.gradeType?.toString().startsWith("TRIMESTER_ONGOING_GRADE"))
            const trimesterInitialKnowledge = gradeData?.filter((grade) => grade.gradeType?.toString() === "TRIMESTER_INITIAL_KNOWLEDGE_GRADE")
            const progressGrade = gradeData?.filter((grade) => grade.gradeType?.toString() === "TRIMESTER_PROGRESS_GRADE")
            const finalExam = gradeData?.filter((grade) => grade.gradeType?.toString() === "TRIMESTER_FINAL_EXAM_GRADE")
            const fullGrade = gradeData?.filter((grade) => grade.gradeType?.toString() === "TRIMESTER_GRADE")
            return [
                {
                ...
                },
                {
                    ...
                    boxdetails: [
                        {
                            label: '',
                            grade: finalExam[0]?.value ? finalExam[0]?.value === -50 ? 'ჩთ' : finalExam[0]?.value : "",
                        },
                    ]
                },
                {
