...
const SmallBox = ({testID, boxdetails, mark}) => {
    ...
    return ( 
        <div className="ib__center column ethical" style={{margin:"unset"}}>
            {boxdetails !== 'თვის ქულა' && boxdetails !== 'გაცდენილი საათები' ?
             <>
                <div style={{lineHeight: '20px'}}>{boxdetails.label}</div>
                <div className="grade__data__api ethical">{boxdetails.grade}</div>
             </>
              :
                <div className="grade__data__api ethical">{boxdetails === 'თვის ქულა' ? mark.monthlyGrade : mark.gacdena}</div>
               }
