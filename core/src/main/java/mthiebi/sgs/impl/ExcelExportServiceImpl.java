...
import mthiebi.sgs.dto.GradeWrapperByStudent;
...
/**
 * POI workbook building for the grade exports. Extracted verbatim from the former {@code ExcelExportController}
 * so the controller only deals with HTTP. The cell indices, merged regions and Georgian labels are part of the
 * expected export layout and must not be changed without agreeing the new layout with the school.
 */
@Service
public class ExcelExportServiceImpl implements ExcelExportService {

    @Override
    public Workbook buildMonthlyExcel(List<GradeComponentWrapper> list, AcademyClass academyClass, Date date, boolean isDecimalSystem) {
        ...
        applyThickBorder(workbook, sheet);
        return workbook;
    }

    @Override
    public Workbook buildSemesterExcel(List<GradeComponentWrapper> list, AcademyClass academyClass, String yearRange, String component, boolean isDecimalSystem) {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet(academyClass.getClassName());
    ...
    @Override
    public Workbook buildAnnualExcel(List<GradeComponentWrapper> list, AcademyClass academyClass, String yearRange, boolean isDecimalSystem) {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet(academyClass.getClassName());
    ...
    @Override
    public Workbook buildDashboardExcel(List<GradeWrapperDto> list, AcademyClass academyClass, Date date, String gradeTypePrefix) {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet(academyClass.getClassName());
...
// Sub-headers for "საშინაო დავალება III"
    ...
    private void applyThickBorder(Workbook workbook, Sheet sheet) {
