...
import mthiebi.sgs.utils.GradePeriods;
import mthiebi.sgs.utils.GradeViewConstants;
...
/**
 * Assembles the grade views consumed by the dashboards. Two intentional conventions run through this class
 * and must be preserved (the frontends depend on them):
 * <ul>
 *   <li>Synthetic behaviour / absence / rating rows use sentinel subject ids – see
 *       {@link GradeViewConstants}.</li>
 *   <li>The {@code Map<Integer, BigDecimal>} returned per subject uses the integer key as a <em>cell
 *       position</em> for the frontend (negative keys for diagnostics/behaviour/absence/computed cells,
 *       positive keys for month columns). See FRONTEND_REFACTOR_NOTES.md for the full key map.</li>
 * </ul>
 * Reporting-calendar quirks (Feb-&gt;Jan, Oct-&gt;Sep, diagnostics-&gt;Dec/Jun) live in {@link GradePeriods}.
 */
@Service
...
public class GradeServiceImpl implements GradeService {
    ...
    public Grade insertStudentGrade(Grade grade, String semester) {
        ...
        calendar.set(Calendar.MONTH, GradePeriods.reportingMonthZeroBased(calendar));
...
//        addAbsenceGradeIfNecessary(grade, academyClass, student, calendar, existing);
    ...
    private Date checkForDiagnostic(Calendar calendar, GradeType gradeType, String semester) {
        calendar.set(Calendar.MONTH, GradePeriods.diagnosticMonthZeroBasedOrDefault(gradeType, calendar.get(Calendar.MONTH)));
    ...
    private List<Grade> fillWithEmptyGradeListOfGradeType(Student student,
        ...
        behaviourSubject.setId(GradeViewConstants.BEHAVIOUR_SUBJECT_ID);
        ...
        ratingSubject.setId(GradeViewConstants.RATING_SUBJECT_ID);
        ...
        junkSubject.setId(GradeViewConstants.ABSENCE_SUBJECT_ID);
